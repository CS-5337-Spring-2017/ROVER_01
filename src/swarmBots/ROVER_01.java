    ...
    private void run() throws IOException, InterruptedException {
        ...
        try {
            ...
            while ( true ) { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                ...
                ////////////For Test Gathering///////////////
//               scienceDetail = new ScienceDetail();
//               scienceDetail.setX(7);
//               scienceDetail.setY(24);
//               scienceDetail.setTerrain(Terrain.SOIL);
//               scienceDetail.setScience(Science.ORGANIC);
                ...
                if ( scienceDetail != null ) {
                    ...
                    if ( scienceDetail.getX() == currentLoc.xpos
                        ...
                        aStar.addScanMap( doScan(), currentLoc, tool1,
                            tool2 );

                        char dirChar = aStar.findPath( currentLoc,
                    ...
                    moveTargetLocation = chooseMoveTargetLocation( scanMapTiles,
                        currentLocInMapTile, currentLoc, mapTileCenter, currentDir);
                 ...
                 // The rover is at the location of science, so gather
                    if ( scanMapTiles[mapTileCenter][mapTileCenter].getScience().toString().equals("ORGANIC") ) {
                        gatherScience( currentLoc );
                        System.out.println( "$$$$$> Gathered science ORGANIC at location "
                            + currentLoc );
                    }
                }
                ...
                try {
                    ...
                    roverDetail.setDriveType( RoverDriveType.valueOf(
                        RoverConfiguration.ROVER_01.getMembers().get( 0 ) ) );
                    roverDetail.setToolType1( RoverToolType.valueOf(
                        RoverConfiguration.ROVER_01.getMembers().get( 1 ) ) );
                    roverDetail.setToolType2( RoverToolType.valueOf(
                        RoverConfiguration.ROVER_01.getMembers().get( 2 ) ) );
    ...
    private MoveTargetLocation chooseMoveTargetLocation(
        ...
        int mapTileCenter, Direction currentDir) {
        ...
        if (!isBlocked(scanMapTiles, getNextCoord(currentDir,currentLocInMapTile))) {
			switch (currentDir) {
			case NORTH:
			...
			if (!isBlocked(scanMapTiles, getNextCoord(Direction.NORTH, currentLocInMapTile)) && !currentDir.equals(Direction.SOUTH)) {
				moveTargetLocation.targetCoord = northCoord;
				moveTargetLocation.d = Direction.NORTH;
			}else if (!isBlocked(scanMapTiles, getNextCoord(Direction.EAST, currentLocInMapTile)) && !currentDir.equals(Direction.WEST)) {
				moveTargetLocation.targetCoord = eastCoord;
				moveTargetLocation.d = Direction.EAST;
			}else if (!isBlocked(scanMapTiles, getNextCoord(Direction.SOUTH, currentLocInMapTile)) && !currentDir.equals(Direction.NORTH)) {
				moveTargetLocation.targetCoord = southCoord;
				moveTargetLocation.d = Direction.SOUTH;
			}else {
				moveTargetLocation.targetCoord = westCoord;
...
//        }
	...
	public Coord getNextCoord(Direction direction, Coord currentLocInMapTile){
		...
		switch (direction) {
			case NORTH: nextCoord = getCoordNorthOf( currentLocInMapTile ); break; 
			case EAST: nextCoord = getCoordEastOf( currentLocInMapTile ); break;
			case WEST: nextCoord = getCoordWestOf( currentLocInMapTile ); break;
			case SOUTH: nextCoord = getCoordSouthOf( currentLocInMapTile ); break;
			default: break;	
		}
